...
* **`auto_row_sep_chars` default changed from `500` to `8192`** — larger scan window for accurate row separator detection on files with wide headers or long first lines. Values below `8192` (and `nil` / `0`) are now rejected and fall back to the default with a warning. **Behavior change:** `nil` / `0` previously documented as "scan whole file" — that semantics is removed; the new hard cap is 64KB regardless.

* **`guess_line_ending` chunked scan** — scans the input in chunks up to a 64KB hard cap, returning as soon as one separator has a clear majority. Near-tie chunk-boundary artifacts no longer cause spurious warnings; only true ties at the hard cap fall back to `"\n"` and emit `:no_clear_row_sep` at `:error` severity.

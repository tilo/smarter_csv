...
**Dominant factor: `auto_row_sep_chars` default `500` → `8192`** — the auto-detection scan window grew 16× larger by default. The benchmark config leaves `row_sep` at `:auto` for every file, so every run reads ~7 KB up front (vs ~500 bytes before) and runs the row-separator regex scan over it. On small / fast files where total parse time is 25–30 ms, that one-time CPU cost shows up as a 5–15% slowdown. On larger files where parse time dominates, the same change is invisible or even net-positive — the wider window often finds a clear majority on the first chunk, avoiding a chunk-grow loop.
...
The gains and regressions in the table above are both consistent with this single cause: files with long lines / lots of work per row absorb the wider scan cost easily; files with short lines / minimal work per row see the cost more visibly.

**Progressive Auto-Detection:** `auto_row_sep_chars` semantics is "initial chunk size" defaults to `512`.** `guess_line_ending` now reads `auto_row_sep_chars` bytes on iter 1, reuses that size on iter 2, then doubles on each subsequent iteration up to `MAX_AUTO_ROW_SEP_CHARS`. With the new default of 512, the read pattern is: `512 → 512 → 1024 → 2048 → 4096 → 8192 → 16384 → 32768`. Common files (clear separator majority within the first ~50 bytes) resolve in iter 1 with 512 bytes of regex scan — back to roughly the 1.16.4 cost. Wide-header / preamble-heavy files escalate as needed. The benchmark numbers in the tables above were taken **before** this fix; a re-run on AC power should show the small-file regressions close.

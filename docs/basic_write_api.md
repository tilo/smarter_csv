...
Pairing `SmarterCSV.generate` with `SmarterCSV.each` on the read side is the idiomatic replacement for Ruby's `CSV.filter`. See [Examples → Filtering and Transforming a CSV File](./examples.md#filtering-and-transforming-a-csv-file) for the full set of patterns, including streaming gzip → gzip pipelines.

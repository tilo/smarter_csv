...
[true, false].each do |acceleration|
  describe "handling of additional trailing column separators with#{acceleration ? ' C-' : 'out '}acceleration" do
    let(:file) { "#{fixture_path}/additional_separator.csv" }

    describe '' do
      let(:data) { SmarterCSV.process(file, acceleration: acceleration) }

...
[true, false].each do |acceleration|
  describe "can handle empty columns with#{acceleration ? ' C-' : 'out '}acceleration" do
    ...
    describe 'with remove_empty_values: true' do
      options = {remove_empty_values: true, acceleration: acceleration}
      it 'has empty columns at end' do
        data = SmarterCSV.process("#{fixture_path}/empty_columns_1.csv", options)
        expect(data.size).to eq 1
        item = data.first
    ...
    describe 'with remove_empty_values: false' do
      options = {remove_empty_values: false, acceleration: acceleration}
      it 'has empty columns at end' do
        data = SmarterCSV.process("#{fixture_path}/empty_columns_1.csv", options)
        expect(data.size).to eq 1
        item = data.first

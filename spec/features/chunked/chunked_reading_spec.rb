...
[true, false].each do |bool|
  describe "chunked reading with#{bool ? ' C-' : 'out '}acceleration" do
    it 'loads_chunk_cornercase_csv_files' do
      6.times do |chunk_size| # test for all chunk-sizes
        options = {chunk_size: chunk_size, remove_empty_hashes: true, acceleration: bool}
    ...
    it 'processes chunks with a block' do
      i = 0
      n = 0
      options = {chunk_size: 2, remove_empty_hashes: true, acceleration: bool}
    ...
    it 'processes blocks without chunking' do
      i = 0
      n = 0
      options = {remove_empty_hashes: true, acceleration: bool}
      SmarterCSV.process("#{fixture_path}/chunk_cornercase.csv", options) do |chunk|
        i += 1

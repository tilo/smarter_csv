...
[true, false].each do |acceleration|
  describe "read binary files with#{acceleration ? ' C-' : 'out '}acceleration" do
    let(:binary_file) { "#{fixture_path}/binary.csv" }

    it 'loads_binary_file_with_comments' do
      options = {col_sep: "\cA", row_sep: "\cB", comment_regexp: /^#/, acceleration: acceleration}
    ...
    it 'loads_binary_file_with_strings_as_keys' do
      options = {col_sep: "\cA", row_sep: "\cB", comment_regexp: /^#/, strings_as_keys: true, acceleration: acceleration}
      data = SmarterCSV.process(binary_file, options)

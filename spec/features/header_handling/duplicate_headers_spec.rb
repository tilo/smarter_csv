  ...
  [true, false].each do |acceleration|
    context "with#{acceleration ? '' : 'out'} acceleration" do
      it 'assigns column_N keys to empty headers using missing_header_prefix (default)' do
        data = SmarterCSV.process("#{fixture_path}/empty_headers.csv", acceleration: acceleration)
      ...
      it 'respects a custom missing_header_prefix' do
        data = SmarterCSV.process("#{fixture_path}/empty_headers.csv", missing_header_prefix: 'field_', acceleration: acceleration)
    ...
    context "with#{acceleration ? '' : 'out'} acceleration" do
      describe 'without special handling / default behavior' do
        it 'does not raise error when duplicate_header_suffix is given' do
          expect do
            SmarterCSV.process("#{fixture_path}/duplicate_headers.csv", {duplicate_header_suffix: '', acceleration: acceleration})
          end.not_to raise_exception
        end

        it 'raises error when user_provided_headers with duplicates are given' do
          expect do
            options = {user_provided_headers: %i[a b c d a], headers_in_file: false, acceleration: acceleration}
        ...
        it 'can remap duplicated headers' do
          options ={key_mapping: {email: :a, firstname: :b, lastname: :c, email2: :d, age: :e}, acceleration: acceleration}
      ...
      describe 'with special handling' do
        context 'when suffix is set to nil' do
          let(:options) { {duplicate_header_suffix: nil, acceleration: acceleration} }
        ...
        context 'with given suffix' do
          let(:options) { {duplicate_header_suffix: '_', acceleration: acceleration} }
        ...
        context 'with different suffix' do
          let(:options) { {duplicate_header_suffix: ':', acceleration: acceleration} }

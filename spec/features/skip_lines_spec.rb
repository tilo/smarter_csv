...
[true, false].each do |acceleration|
  describe ":skip_lines option with#{acceleration ? ' C-' : 'out '}acceleration" do
    it 'loads_csv_file_skipping_lines' do
      options = {skip_lines: 3, acceleration: acceleration}
    ...
    it 'loads_csv_with_user_defined_headers' do
      options = {skip_lines: 3, headers_in_file: true, user_provided_headers: %i[a b c d e f], acceleration: acceleration}
      data = SmarterCSV.process("#{fixture_path}/skip_lines.csv", options)
      expect(data.size).to eq 4

  ...
  [true, false].each do |acceleration|
    context "with#{acceleration ? '' : 'out'} acceleration" do
      ...
      describe "with given row_sep" do
        let(:options) { {row_sep: sep, acceleration: acceleration}}
      ...
      describe "with auto row_sep" do
        let(:options) { {row_sep: :auto, acceleration: acceleration}}
        let(:data) { SmarterCSV.process("#{fixture_path}/#{file}", options) }
      ...
      it 'should process a file with more quoted text carriage return characters (\r) than line ending characters (\n)' do
        row_sep = "\n"
        text_sep = "\r"
        data = SmarterCSV.process("#{fixture_path}/carriage_returns_quoted.csv", {row_sep: row_sep, acceleration: acceleration})
      ...
      describe "with given row_sep" do
        let(:options) { {with_line_numbers: true, row_sep: sep, acceleration: acceleration}}
        let(:data) { SmarterCSV.process("#{fixture_path}/#{file}", options) }
      ...
      describe "with auto row_sep" do
        let(:options) { {with_line_numbers: true, row_sep: :auto, acceleration: acceleration}}
        let(:data) { SmarterCSV.process("#{fixture_path}/#{file}", options) }

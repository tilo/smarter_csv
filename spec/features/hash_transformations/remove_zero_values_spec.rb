  ...
  [true, false].each do |acceleration|
    context "acceleration: #{acceleration}" do
      it 'removes zero values' do
        options = {remove_zero_values: true, remove_empty_values: true, acceleration: acceleration}
      ...
      it 'keeps zero values when remove_zero_values is false' do
        options = {remove_zero_values: false, remove_empty_values: true, acceleration: acceleration}
        data = SmarterCSV.process("#{fixture_path}/basic.csv", options)
